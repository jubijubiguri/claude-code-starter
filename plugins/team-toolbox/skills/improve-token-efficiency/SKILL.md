...
스크립트 경로는 **이 SKILL.md가 있는 스킬 디렉터리 기준** `scripts/` 입니다 (플러그인으로 설치된 경우에도 동일 — 절대경로를 하드코딩하지 말 것). 임시 파일 경로는 OS에 맞게 선택한다.

```bash
# macOS / Linux
python3 <skill-dir>/scripts/analyze_sessions.py --repo "$(pwd)" --out /tmp/session_analysis.json

# Windows (PowerShell — python3 명령이 없는 경우가 많음. /tmp 대신 $env:TEMP 사용)
python <skill-dir>\scripts\analyze_sessions.py --repo "$PWD" --out $env:TEMP\session_analysis.json
```
...
```bash
# macOS / Linux
python3 <skill-dir>/scripts/build_dashboard.py --input /tmp/session_analysis.json --out /tmp/efficiency_report.html
open /tmp/efficiency_report.html

# Windows (PowerShell)
python <skill-dir>\scripts\build_dashboard.py --input $env:TEMP\session_analysis.json --out $env:TEMP\efficiency_report.html
start $env:TEMP\efficiency_report.html
```
