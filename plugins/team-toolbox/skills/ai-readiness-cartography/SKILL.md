...
스크립트 경로는 **이 SKILL.md가 있는 스킬 디렉터리 기준** `scripts/score.py` 입니다 (플러그인으로 설치된 경우에도 동일 — 절대경로를 하드코딩하지 말 것).

```bash
# macOS / Linux
python3 <skill-dir>/scripts/score.py <repo-path> --json <output-path>/ai-readiness-score.json

# Windows (python3 명령이 없는 경우가 많음 — python 또는 py -3 사용)
python <skill-dir>\scripts\score.py <repo-path> --json <output-path>\ai-readiness-score.json
```

스크립트는 stdlib only (deps 없음, Python 3.10+). Python이 없으면 사용자에게 설치를 안내하고 중단할 것. 출력:
...
```bash
start <output-path>\ai-readiness-map.html  # Windows
